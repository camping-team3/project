...
-- ==========================================================
-- [추가] 페이징 테스트를 위한 대량 더미 데이터 (Total 20+ records per table)
-- ==========================================================

-- 1. 사용자 추가 (user_tb) - 총 20명 확보
INSERT INTO user_tb (username, password, name, email, phone, role, status, created_at) VALUES 
...
-- 3. 포토 갤러리 추가 (gallery_tb) - 총 22건 확보
INSERT INTO gallery_tb (title, content, created_at) VALUES 
...
-- 5. 이용 후기 추가 (review_tb) - 총 21건 확보
INSERT INTO review_tb (user_id, reservation_id, rating, content, created_at) VALUES 
(3, 2, 4, '조용하고 공기도 좋아서 힐링 제대로 하고 왔습니다.', DATEADD('DAY', -15, NOW())),
(4, 3, 5, '시설이 정말 깨끗하고 관리가 잘 되어 있어요. 다음에도 꼭 올 거예요!', DATEADD('DAY', -14, NOW())),
(5, 4, 3, '그늘이 좀 부족해서 낮에는 좀 더웠지만 전반적으로 만족합니다.', DATEADD('DAY', -13, NOW())),
(2, 1, 5, '사장님이 너무 친절하셔서 기분 좋게 머물다 갑니다.', DATEADD('DAY', -12, NOW())),
(3, 2, 4, '가족들과 좋은 추억 만들고 갑니다. 아이들이 좋아하네요.', DATEADD('DAY', -11, NOW())),
(4, 3, 2, '화장실 온수가 잘 안 나와서 조금 불편했습니다.', DATEADD('DAY', -10, NOW())),
(5, 4, 5, '별이 정말 많이 보여요! 밤하늘이 예술입니다.', DATEADD('DAY', -9, NOW())),
(2, 5, 4, '산책로가 잘 되어 있어서 아침마다 걷기 좋았어요.', DATEADD('DAY', -8, NOW())),
(3, 1, 5, '사이트 간격이 넓어서 프라이버시가 잘 보호되는 느낌입니다.', DATEADD('DAY', -7, NOW())),
(4, 2, 4, '전체적으로 깔끔하고 조용해서 쉬기 좋습니다.', DATEADD('DAY', -6, NOW())),
(5, 3, 1, '매너타임 관리가 전혀 안 돼서 밤새 시끄러워 잠을 설쳤습니다.', DATEADD('DAY', -5, NOW())),
(2, 4, 4, '경치가 너무 아름답고 시설도 현대적이라 좋았어요.', DATEADD('DAY', -4, NOW())),
(3, 5, 5, '지금까지 가본 캠핑장 중 최고입니다. 강력 추천해요!', DATEADD('DAY', -3, NOW())),
(4, 1, 3, '가성비는 좀 떨어지는 느낌이지만 뷰는 좋습니다.', DATEADD('DAY', -2, NOW())),
(5, 2, 4, '글램핑 시설이 호텔 수준이네요. 몸만 와도 돼서 편했습니다.', DATEADD('DAY', -1, NOW())),
(2, 3, 5, '카라반 내부가 너무 아늑하고 깨끗해요.', NOW()),
(3, 4, 4, '주변에 즐길 거리도 많고 접근성도 좋네요.', NOW()),
(4, 5, 5, '재방문 의사 200%입니다! 너무 좋았어요.', NOW()),
(5, 1, 2, '기대가 너무 컸는지 생각보다 평범했습니다.', NOW()),
(2, 2, 4, '깔끔한 시설과 친절한 서비스에 만족합니다.', NOW());


-- 6. 예약 추가 (reservation_tb) - 총 25건 확보
INSERT INTO reservation_tb (user_id, site_id, check_in, check_out, total_price, people_count, visitor_name, visitor_phone, status, created_at) VALUES 
