...
- [ ] **3-2. 예약 변경 요청 기능 및 가예약(Lock) 로직 구현**
    ...
    - 서비스 로직: `ReservationService`에서 중복 예약 체크 시 `ReservationChangeRequest`의 `PENDING` 상태 사이트도 필터링하도록 수정 (가예약 Lock 처리)
- [ ] **3-3. 예약 취소 요청 기능 구현**
...
- [ ] **4-1. 관리자 예약 목록 필터링 확장**
    ...
    - 기능: '변경 요청 중', '취소 요청 중' 건들만 모아보기 필터 추가
...
- [ ] **4-3. 승인/거절 처리 API 및 비즈니스 로직 구현**
    - 승인: `Reservation` 엔티티의 실제 날짜/사이트 업데이트 및 요청 상태 `APPROVED`로 변경 (PRG 패턴 적용)
    - 거절: 모달창으로 사유 입력받아 `REJECTED` 처리 및 사유 저장

## 4단계: 통합 테스트 및 검증

### Task 5: 시스템 안정성 및 규칙 검증
- [ ] **5-1. 가예약(Lock) 정합성 테스트**: 변경 요청 중인 사이트에 대해 타 사용자의 중복 예약 시도 차단 여부 확인
