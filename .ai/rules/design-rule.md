...
모든 UI(Mustache, HTML, CSS) **신규 구현** 작업에 들어가기 전, AI는 반드시 다음 과정을 거쳐야 한다. **(단, 이미 생성된 페이지의 단순 스타일 수정, 버그 픽스, 사소한 수치 조정 시에는 아래 '절차'를 생략하거나 간소화할 수 있으나, '1. 디자인 토큰 전략' 및 '2. 공통 컴포넌트 표준'은 어떠한 경우에도 반드시 준수해야 한다.)**
...
3. **목업 확인 (Mockup Preview)**: 실제 파일에 코드를 적용하기 전, 다음 두 가지 형식을 순서대로 채팅창에 반드시 제시한다.
   - **전체 소스 코드 (Full Code)**: Mustache 템플릿의 전체 소스 코드를 마크다운 코드 블록으로 가장 먼저 제시한다.
   - **레이아웃 구조 (Box Diagram)**: 제시된 코드가 구현할 UI의 구조를 텍스트 박스를 활용하여 시각화한다. **(아래 예시는 '보고 형식'에 대한 가이드일 뿐이며, 실제 상자 구조는 구현할 페이지의 디자인에 따라 자유롭게 구성한다.)**
     ```text
     +---------------------------------------------------------+
...
4. **최종 승인 및 기능 구현 연결**: 
   - 사용자가 UI 목업을 최종 승인하면 실제 파일(`replace`, `write_file`)을 수정한다.
   - **[중요]** 해당 UI와 연동되는 **백엔드(Java) 기능 구현**이 필요한 경우, UI 승인 직후 바로 코드를 작성하지 않는다. 
   - 반드시 **`plan` 스킬의 [B-1. 태스크 계획 제안] 단계**로 넘어가 백엔드 구현 전략을 먼저 텍스트로 설명하고 사용자의 검토를 받는다.
