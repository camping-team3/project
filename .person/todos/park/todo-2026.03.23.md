...
- [x] **5-1. 가예약(Lock) 정합성 테스트**: 변경 요청 중인 사이트(원본 및 신규)에 대해 타 사용자의 중복 예약 시도 차단 여부 확인
- [x] **5-2. 비즈니스 규칙 및 예외 처리 검증**: 이용일(체크인)이 이미 지난 예약에 대해 변경/취소 요청 시도 시 예외 처리 확인
- [x] **5-3. 통합 시스템 정합성 및 데이터 영속성 최종 검증**
  - [x] **Phase 2 & 3 통합 확인**: 기본 예약/결제 흐름과 신규 확장 기능(변경/취소/승인/거절)의 상호작용 및 UI/UX 정합성 전수 조사
  - [x] **서버 안정성 점검**: 모든 기능 수행 시 서버 콘솔 내 예외(Exception) 발생 여부 및 로그 모니터링
  - [x] **데이터 영속성 확인**: 예약 신청(POST), 관리자 처리(POST) 후 DB 내 상태값(`status`, `rejectionReason` 등) 및 연관 엔티티 반영 여부 최종 확인
- [x] **5-4. 최종 리포트 작성 및 TODO.md 완료 체크**
