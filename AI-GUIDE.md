...
## 5. AI-CONTEXT & Automatic Rules
- 코드 작업 전 해당 디렉토리에 `AI-CONTEXT.md`가 있으면 먼저 읽어라.
- **[자동 룰 호출]**: 아래 조건 충족 시 해당 룰을 **자동으로 활성화**하고 절차를 준수한다.
  - **신규 UI 생성**: 새로운 `.mustache`, `.html` 파일을 만들 때는 반드시 `.ai/rules/design-rule.md`를 먼저 읽고 '🚨 UI 작업 시작 전 필수 절차'를 즉시 실행한다.
